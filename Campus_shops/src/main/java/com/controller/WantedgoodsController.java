...
public class WantedgoodsController {
    ...
    public ResultVo showWantGoodsLits(@PathVariable("page") Integer page, @PathVariable("wantstatus") Integer wantstatus, ModelMap modelMap) {
        //查询所有求购列表
        List<Wantedgoods> wantedgoodsList = wantedgoodsService.queryWantgoodsByStatus((page - 1) * 16, 16, wantstatus,null);
     ...
     * 根据状态显示数据
     */
    @GetMapping("/showmywglist/{wantstatus}")
    @ResponseBody
    public LayuiPageVo showMyWgList(@PathVariable("wantstatus") Integer wantstatus, int limit, int page, HttpSession session) {
        //通过session获取用户id
        String userid = (String) session.getAttribute("userid");
        Wantedgoods wg = new Wantedgoods().setWantstatus(wantstatus).setUserid(userid);
        //查询所有待审核求购列表
        if (wantstatus == 100) {
            //全部商品，则wantstatus传入null
            List<Wantedgoods> wantedgoodsList = wantedgoodsService.queryWantgoodsByStatus((page - 1) * limit, limit, null,userid);
            Integer dataNumber = wantedgoodsService.queryWantgoodsCountByStatus(wg);
            return new LayuiPageVo("", 0, dataNumber, wantedgoodsList);
        } else {
            List<Wantedgoods> wantedgoodsList = wantedgoodsService.queryWantgoodsByStatus((page - 1) * limit, limit, wantstatus,userid);
            Integer dataNumber = wantedgoodsService.queryWantgoodsCountByStatus(wg);
            return new LayuiPageVo("", 0, dataNumber, wantedgoodsList);
        }
    }
